...
batch_census <- function(unique_addresses,
  ...
  results <- utils::read.csv(
    text = response$content, header = FALSE,
...
batch_here <- function(unique_addresses, lat = "lat", long = "long", timeout = 20, full_results = FALSE, custom_query = list(),
  ...
  if (full_results) {
    outcols <- c(
      "displayLatitude,displayLongitude,locationLabel",
  ...
  body <- paste(
    paste0("recID", query_parameters[["indelim"]], "searchText\n"),
    paste(address_df$recId, query_parameters[["indelim"]],
      address_df$searchText,
      collapse = "\n"
    )
  )
  ...
  while (!current_status %in% c("cancelled", "failed", "completed")) {
    ...
    status <- httr::GET(
      url = paste0(api_url, "/", here_request_id),
      query = list(
        action = "status",
        apiKey = get_key("here")
      )
  ...
  batch_results <-
    httr::GET(
      url = paste0(api_url, "/", here_request_id, "/result"),
      query = list(
        apiKey = get_key("here"),
...
batch_mapquest <- function(unique_addresses, lat = "lat", long = "long",
  ...
  if (nrow(address_df) == 1) {
    results <- geo(
      address = address_df[["address"]], method = "mapquest",
