...
geo_cascade <- function(..., cascade_order = c("census", "osm")) {
  ...
  if (any(na_indices)) {
    ...
    if (is.null(names(initial_parameters))) {
      retry_addresses <- input[na_indices]
    } else {
      retry_addresses <- lapply(
        input[names(input) %in% pkg.globals$address_arg_names],
        function(x, bools) x[bools], na_indices
      )
    }

    retry_results <- do.call(geo, c(
      retry_addresses,
  ...
  if (unique_results_requested == TRUE) {
    return(unique(combi))
  } else {
    return(combi)
  }
