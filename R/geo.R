...
geo <- function(address = NULL,
  ...
  if (method == "cascade") {
    ...
    legal_parameters <- unique(c(
      get_generic_parameters(cascade_order[1]),
  ...
  if (method == "cascade") {
    ...
    return(do.call(
      geo_cascade,
      c(
        all_args[!names(all_args) %in% c("method", "param_error", "batch_limit_error")],
  ...
  if ((limit != 1) & (!("limit" %in% legal_parameters))) {
    illegal_limit_message <- paste0(
      'The limit parameter must be set to 1 (the default) because the "', method, '" ',
      "method API service does not support a limit argument.\n\n",
      "See ?api_parameter_reference for more details."
    )

    if (param_error == TRUE) {
      stop(illegal_limit_message)
  ...
  if (is.null(api_url)) {
    api_url <- get_api_url(method,
      reverse = FALSE, return_type = return_type,
